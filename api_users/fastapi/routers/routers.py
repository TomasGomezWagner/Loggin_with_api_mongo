...
@router.get('/{id}', response_description='Get single book', response_model=Book)
def find_book(id:str, request:Request):
    if (book := request.app.database['books'].find_one({'_id':id})) is not None:
        return book
    

@router.get('/title/{title}', response_description='Get single book', response_model=Book)
...
def update_book(id, request:Request, book:BookUpdate=Body(...)):
